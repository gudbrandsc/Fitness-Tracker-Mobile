...
class ExpensesPage extends Component {
  ...
  retrieveDetails = async () => {
    try {
      ...
      this.onLoginFail("Can't get Data. Please check internet connectivity.");
    }
  };
  ...
  addExpense() {
    if (this.state.expenseAmount !== "" && this.state.expenseText !== "") {
      try {
        ...
        console.log(error);
        this.onLoginFail("Request failed. Please check internet connectivity.");
      }
    } else {
      ...
      if (this.state.expenseAllData && this.state.expenseAllData.length > 0) {
        return (
          <ScrollView style={styles.scrollViewStyle}>
            <ExpensesList
              expenses={this.state.expenseAllData}
        ...
        return (
          <View style={{ paddingTop: 10 }}>
            <Text style={{ textAlign: "center" }}>
              You dont have any saved expenses yet
            </Text>
          </View>
        );
      }
  ...
  render() {
    return (
      <View style={styles.containerStyle}>
        <TouchableOpacity
          style={styles.touchViewContainer}
          onPress={() =>
            this.setState({ activeExpenseList: !this.state.activeExpenseList })
          }
        >
          <View
            style={[
              styles.touchViewStyle,
              this.state.activeExpenseList
                ? styles.showExpenseList
                : styles.showAddExpenseView
            ]}
          >
            {this.getTouchtext()}
