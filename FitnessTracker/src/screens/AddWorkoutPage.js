...
export default class AddWorkoutPage extends Component {
  ...
  componentDidMount(){
    ...
    if(Platform.OS === 'ios'){
      ...
      setIP = 'IP FOR android';
    }

    this.retrieveDetails();
    axios.get('http://' + setIP + ':8000/api/workoutcategories').then(response => this.setState({ workouts: response.data })).then(this.checkSearchResp.bind(this));
  }
